    ...
    this.x += dt * this.speed;
        ...
        setTimeout(allEnemies.push(newEnemy),Math.floor(Math.random() * 2000) +1);
            ...
            allEnemies.forEach(function(e){
                if(e.x > player.x-50 && e.x < player.x+51 && e.y > player.y && e.y < player.y +76)
                {
                    player.update(player.x =202,player.y = 373.5);
                    if(player.score > 0){
                        player.score -= 10;
                    }
                }
...
Player.prototype.render = function(){
    ...
    if(gameTime === 0 && player.lose === false){
        player.lose = true;
        player.playerLose();
    }
};
...
var Gem = function(){
    ...
    this.y =  (Math.floor(Math.random() * 3)+1) * 75;
    this.gemcollection = ["images/Gem Blue.png","images/Gem Green.png","images/Gem Orange.png"];
    this.sprite =this.gemcollection[Math.floor(Math.random()*3)];
};
Gem.prototype.update=function(){
   if(allGems.length > 1){
    ...
    allGems.forEach(function(e){
        if(e.x > player.x-50 && e.x < player.x+51 && e.y > player.y && e.y < player.y +76)
        {
            player.score += 20;
            let index =  allGems.indexOf(e);
